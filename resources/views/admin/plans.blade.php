...
<p class="text-sm text-muted">Preço, benefícios, limites e período de teste são configuráveis aqui — nunca no código.</p>
<div class="mt-5 grid gap-4 md:grid-cols-3">
    @foreach($plans as $p)
        <div class="card">
            <div class="flex justify-between"><h2 class="font-semibold">{{ $p->name }}</h2><span class="text-xs text-muted">{{ $p->code }}</span></div>
            ...
            <p class="text-xs text-muted">Trial: {{ $p->trial_days }} dias · {{ $p->is_active ? 'ativo' : 'inativo' }}</p>
...
<form method="POST" action="{{ route('admin.plans.store') }}" class="card mt-5 space-y-3">
    ...
    <div class="grid gap-3 sm:grid-cols-3">
        ...
        <div><label class="label">Dias de teste</label><input class="input" name="trial_days" type="number" min="0" value="0"></div>
    ...
    <div><label class="label">Limites (JSON) — -1 = ilimitado</label><textarea class="input font-mono" name="limits" rows="4">{"fullExamsPerMonth": -1, "essaysPerMonth": 4, "studyPlan": true, "tutor": true, "errorNotebook": true}</textarea></div>
