...
<form method="POST" action="{{ route('login') }}" class="mt-6 space-y-4">
    ...
    <div class="flex items-center justify-between text-sm">
        ...
        <a href="{{ route('password.request') }}" class="text-[#c4b5fd] hover:underline">Esqueci a senha</a>
    </div>
    <button class="btn-primary w-full">Entrar</button>
</form>
<p class="mt-4 text-center text-sm text-muted">Não tem conta? <a href="{{ route('register') }}" class="text-[#c4b5fd] hover:underline">Criar conta gratuita</a></p>
@endsection
